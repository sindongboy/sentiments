...
public final class Collocation {
     ...
     */
    public static Collocation getInstance() {
        if (instance == null) {
    ...
    private Collocation() {
        ...
        // Collocation Dictionary loading.
        dict = new TokenTrie();
        topDict = new TokenTrie();
    }

    public void dictionaryReLoading() {
        // dictionary reset
        dict = new TokenTrie();
        topDict = new TokenTrie();
        this.init();
     ...
     */
    public Pair[] process(String[] tokens, String[] postags, int iter, boolean addEndMark) {
        ...
        Pair[] result = null;
        Collections.addAll(toks, tokens);
        Collections.addAll(tags, postags);
    ...
    /**
     * Collocation Dictionary loading and Init.
     */
    public void init() {
        Configuration config = Configuration.getInstance();
        File collocationDictFile = new File(config.getResource(COLLOCATION_DICT_NAME, Configuration.CLASSPATH_LOAD).getFile());

        BufferedReader reader;
        boolean top;

        try {
            reader = new BufferedReader(new FileReader(collocationDictFile));
            String line;
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.startsWith(TOP_PRIORITY)) {
                ...
                // field delim : "+"
                // value delim : "==>"
                int offset = line.indexOf(VALUE_DELIM);
                if (offset < 0) {
                    System.err.println("[WARNING] wrong format : " + line);
                    continue;
                }
                String key = line.substring(0, offset);
                String value = line.substring(offset + VALUE_DELIM.length());
            ...
            reader.close();
            LOGGER.info("collocation dictionary is loading done");
