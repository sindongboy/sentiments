...
public final class KeyTerm {
     ...
     */
    public static KeyTerm getInstance() throws IOException {
        if(instance == null) {
    ...
    // constructor
    private KeyTerm() throws IOException {
        ...
        // set boundary tag
        boundaryTag = prop.getProperty(BOUNDARY_TAG);
        LOGGER.debug("boundary tag: " + boundaryTag);
        // set sentence boundary tag
        sentenceBoundaryTag = prop.getProperty(SENTENCE_END_TAG);
        LOGGER.debug("sentence boundary tag: " + sentenceBoundaryTag);
        // set negation tag
        negationTag = prop.getProperty(NEGATION_TAG);
        LOGGER.debug("negation tag: " + negationTag);

        // set front negation terms
        String[] frontEnds = prop.getProperty(FRONT_NEGATION_LEXS).split(",");
        ...
        // set keyterm postags
        String[] kTags = prop.getProperty(KEYTERM_POSTAG).split(",");
