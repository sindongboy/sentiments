...
public final class Negation {
     ...
     */
    public static boolean isNegated(Pair[] terms, int tIdx) throws IOException {
     ...
     */
    public static boolean isNegated(String[] tokens, String[] postags, int tIdx) throws IOException {
        return check(tokens, postags, tIdx);
    }

    private static boolean check(String[] tokens, String[] tags, int tIdx) throws IOException {
        KeyTerm keyterm = KeyTerm.getInstance();
        if (!(tags[tIdx].equals(NLPTags.VA) || tags[tIdx].equals(NLPTags.VV))) { LOGGER.debug("negation check isn't applicable to \"" + tokens[tIdx] + "/" + tags[tIdx] + "\"");
            return false;
        }
